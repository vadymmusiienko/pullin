...
// Updated interface to support both types of requests with fromGroup field and hasSeen property
...
export default function RequestsPage() {
  ...
  // Function to mark incoming requests as read
  const markRequestsAsRead = async (requests: RequestData[]) => {
    if (!user || requests.length === 0 || markedAsRead) return;
    
    try {
      const batch = writeBatch(db);
      let updateCount = 0;

      // Filter requests that haven't been seen yet
      const unseenRequests = requests.filter(req => req.hasSeen !== true);
      
      if (unseenRequests.length === 0) {
        console.log("No unread requests to mark as read");
        return;
      }

      // Update each request with hasSeen: true
      ...
      await batch.commit();
      console.log(`Marked ${updateCount} requests as read`);
      setMarkedAsRead(true);
    } catch (err) {
      console.error("Error marking requests as read:", err);
      // Don't set an error to the UI here, as this is a background operation
    }
  };
  ...
  useEffect(() => {
    ...
    const unsubscribeIncomingUser = onSnapshot(incomingUserRequests, (snapshot) => {
      ...
      setIncomingRequests(prevRequests => {
        ...
        const filteredRequests = prevRequests.filter(req => req.fromGroup !== true);
        const combinedRequests = [...filteredRequests, ...requests];
        
        // After setting the state with all incoming requests, mark them as read
        // We use setTimeout to ensure the state update happens first
        setTimeout(() => markRequestsAsRead(combinedRequests), 500);
  ...
  // Effect to mark requests as read when all incoming requests are loaded
  useEffect(() => {
    if (!loading && incomingRequests.length > 0 && !markedAsRead) {
      markRequestsAsRead(incomingRequests);
    }
  }, [incomingRequests, loading, markedAsRead]);
  ...
  const renderIncomingRequestItem = (req: RequestData) => {
    // Determine if this is a new request (not seen yet)
    const isNew = req.hasSeen !== true;
