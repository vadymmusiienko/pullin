...
import { db } from '@/lib/firebase/firebaseConfig'; // Assuming db is exported from your config
...
// Interface matching editable fields + potential display fields
...
export default function EditProfilePage() {
  ...
  const [instagramHandle, setInstagramHandle] = useState(''); // <-- New state for Instagram
  ...
  const fetchProfileData = useCallback(async (userId: string) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        setInstagramHandle(data.instagramHandle || ''); // <-- Fetch Instagram handle
        setEmail(data.email || '');
  ...
  const handleUpdateProfile = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const numericGradYear = graduationYear.trim() === '' ? null : parseInt(graduationYear, 10);
    if (graduationYear.trim() !== '' && (isNaN(numericGradYear!) || graduationYear.length !== 4)) {
        setError('Graduation year must be a valid 4-digit number or empty.');
        setIsUpdating(false);
        return;
    }
    ...
    const cleanedInstagramHandle = instagramHandle.trim().replace(/^@/, ''); // Remove leading @ if present
    ...
    const dataToUpdate: Partial<UserProfileData & { updatedAt: FieldValue }> = {
      name: name.trim(),
      bio: bio.trim(),
      ...
      pfpUrl: pfpUrl.trim(),
      instagramHandle: cleanedInstagramHandle, // <-- Add cleaned Instagram handle
 ...
 if (authLoading || isFetching) {
  ...
  if (!user) {
    return (
        <div className="flex justify-center items-center min-h-screen bg-gradient-to-br from-teal-400 to-blue-500">
            <p className="text-white text-xl">Please sign in.</p>
        </div>
    );
  }
             ...
             <div>
               ...
               <div className="relative rounded-lg shadow-sm">
                    ...
                    <input
                        ...
                        // Optional: Add pattern for basic client-side hint, server validates too
                        // pattern="[a-zA-Z0-9._]{1,30}"
                        />
               </div>
                <p className="text-xs text-gray-500 mt-1">Enter your Instagram username (without @).</p>
             </div>
             {/* --- End Instagram Handle Input --- */}


            {/* Graduation Year */}
            <div>
              <label htmlFor="graduationYear" className="block text-sm font-medium text-gray-700 mb-1">Graduation Year</label>
              <input type="number" id="graduationYear" name="graduationYear" value={graduationYear} onChange={(e) => setGraduationYear(e.target.value)} placeholder='YYYY' min="2000" max="2050" step="1"
